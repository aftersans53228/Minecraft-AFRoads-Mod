    ...
    public void render(RoadLightEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (blockEntity.getCachedState().get(Properties.HORIZONTAL_FACING)==SOUTH){
            ...
            if (blockEntity.getCachedState().get(RoadLight.LightType)==0){
                MinecraftClient.getInstance().getItemRenderer().renderItem(RoadLightBulbCold, ModelTransformation.Mode.GROUND, 15728880, OverlayTexture.DEFAULT_UV, matrices, vertexConsumers, 0);
            }
            else{
                MinecraftClient.getInstance().getItemRenderer().renderItem(RoadLightBulbWarm, ModelTransformation.Mode.GROUND, 15728880, OverlayTexture.DEFAULT_UV, matrices, vertexConsumers, 0);
            }
