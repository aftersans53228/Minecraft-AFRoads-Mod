    ...
    public void render(TrafficLightEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        */
        ...
        switch (blockEntity.getCachedState().get(Properties.HORIZONTAL_FACING)){
            ...
            default:
                FabroadsMod.LOGGER.info("Unexpected traffic light orientation state.");
                matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(0));

        }
        //选择渲染类型
        switch (blockEntity.getCachedState().get(TrafficLight.TrafficType)) {
            case 0:
            ...
            default:
                FabroadsMod.LOGGER.info("Unexpected traffic light color state.");
